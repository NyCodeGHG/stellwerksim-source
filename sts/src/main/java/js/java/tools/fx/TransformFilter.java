   ...
   public BufferedImage filter(BufferedImage src, BufferedImage dst) {
      ...
      if (dst == null) {
         ...
         dst = new BufferedImage(
            dstCM, dstCM.createCompatibleWritableRaster(this.transformedSpace.width, this.transformedSpace.height), dstCM.isAlphaPremultiplied(), null
         );
      }
   ...
   private int getPixel(int[] pixels, int x, int y, int width, int height) {
      if (x >= 0 && x < width && y >= 0 && y < height) {
         ...
         switch(this.edgeAction) {
            ...
            case 3:
               return pixels[ImageMath.clamp(y, 0, height - 1) * width + ImageMath.clamp(x, 0, width - 1)] & 16777215;
         }
      }
   }
            ...
            if (!(out[0] < 0.0F) && srcX < srcWidth && !(out[1] < 0.0F) && srcY < srcHeight) {
               ...
               switch(this.edgeAction) {
                  ...
                  case 2:
                     ...
                     break;
                  case 3:
                     p = inPixels[ImageMath.clamp(srcY, 0, srcHeight - 1) * srcWidth + ImageMath.clamp(srcX, 0, srcWidth - 1)] & 16777215;
               }

...
public final class DimConstraint implements Externalizable {
   ...
   public UnitValue getAlignOrDefault(boolean isCols) {
      if (this.align != null) {
         ...
         return !this.fill && PlatformDefaults.getDefaultRowAlignmentBaseline() ? UnitValue.BASELINE_IDENTITY : UnitValue.CENTER;
      }
   }
   ...
   int[] getComponentGaps(
      ContainerWrapper parent, ComponentWrapper comp, BoundSize adjGap, ComponentWrapper adjacentComp, String tag, int refSize, int adjacentSide, boolean isLTR
   ) {
      ...
      if (gap == null) {
         return hasGap ? new int[]{0, 0, -2147471302} : null;
      } else {
         int[] ret = new int[3];
